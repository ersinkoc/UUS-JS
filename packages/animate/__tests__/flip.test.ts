...
describe('FLIP Package', () => {
  ...
  describe('FLIP animation behavior', () => {
    it('should handle position changes', () => {
      ...
      (global.MutationObserver as any).mockImplementation((callback: MutationCallback) => {
  ...
  describe('FLIP edge cases and error handling', () => {
    ...
    it('should handle FLIP with only position changes', () => {
      ...
      mockGetBoundingClientRect.mockImplementation(() => {
        callCount++;
        if (callCount === 1) {
          return { left: 0, top: 0, width: 100, height: 100, right: 100, bottom: 100, x: 0, y: 0 };
        } else {
          return { left: 50, top: 50, width: 100, height: 100, right: 150, bottom: 150, x: 50, y: 50 };
        }
      });
    ...
    it('should handle FLIP with scale and opacity options', () => {
      ...
      mockGetBoundingClientRect.mockImplementation(() => {
        ...
        if (rectCallCount === 1) {
          ...
          return { left: 10, top: 10, width: 200, height: 200, right: 210, bottom: 210, x: 10, y: 10 };
        }
      });
