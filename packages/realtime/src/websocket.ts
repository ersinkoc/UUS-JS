 ...
 */
export function createWebSocket(options: WebSocketOptions = {}): RealtimeConnection {
  ...
  async function connect(): Promise<void> {
    ...
    try {
      ...
      let authData = {};
      if (options.auth) {
        authData = typeof options.auth === 'function' 
          ? await options.auth() 
          : options.auth;
      }
      ...
      ws.onclose = (event) => {
        ...
        if (opts.reconnect.enabled && reconnectAttempts < opts.reconnect.attempts) {
          scheduleReconnect();
        }
      };
