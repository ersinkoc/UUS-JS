 ...
 */
export async function loadLocaleMessages(locale: string, basePath = '/locales'): Promise<LocaleMessages> {
  try {
 ...
 */
export async function loadLocaleMessagesChunk(locale: string): Promise<LocaleMessages> {
  ...
  console.warn(`Dynamic import for locale ${locale} not implemented. Use loadLocaleMessages() instead.`);
  return {};
}
...
export async function loadLocaleMessagesCDN(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load locale ${locale} from CDN: ${response.status}`);
    }
    
    return await response.json();
  } catch (error) {
    console.warn(`Failed to load locale messages from CDN for ${locale}:`, error);
    return {};
  }
}
  ...
  async preloadAll(locales: string[]): Promise<Record<string, LocaleMessages>> {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        console.warn(`Failed to preload locale ${locales[index]}:`, result.reason);
      }
    });
