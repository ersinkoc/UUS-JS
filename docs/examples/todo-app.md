  ...
  <div id="app" uus-state="{ 
    ...
    <div class="app" uus-animate="fadeIn">
      <div class="header">
        <h1>✓ My Todos</h1>
        <div class="stats">
          <span uus-text="todos.filter(t => !t.completed).length"></span> active,
          <span uus-text="todos.filter(t => t.completed).length"></span> completed
        </div>
      </div>
      ...
      <div class="footer" uus-show="todos.length > 0">
        <span>
          ...
          <span uus-text="todos.filter(t => !t.completed).length === 1 ? 'item' : 'items'"></span> left
        </span>
        <button 
...
@keyup.escape="..."    <!-- Escape key handler -->
...
```html
uus-show="todos.length > 0"
uus-if="editingId === todo.id"
```
